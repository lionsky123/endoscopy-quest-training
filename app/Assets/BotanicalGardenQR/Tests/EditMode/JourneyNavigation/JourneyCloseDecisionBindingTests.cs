    ...
    public sealed class JourneyCloseDecisionBindingTests
    {
        ...
        public void ClinicalSixStopCourseAdvancesWithoutAtlasOrCollection(bool skip)
        {
            ...
            using var binding = new JourneyCloseDecisionBinding(source, session, journey, intents,
                new TestNextPointPromptPresenter(), collection, catalog, collectionsEnabled: false);
            ...
            for (int station = 0; station < map.points.Length; station++)
            {
                ...
                else binding.AcceptObservationCompleted(new ObservationCompletedFact(content, session, entry.TargetSceneId,
                    ObservationCompletionKind.Confirmation, "observed"));
                ...
                guidance.Refresh(journey.CurrentState.CompletionRevision, default, navigation.HasFrame, false);
        ...
        public void ActualContentCompletionAdvancesGeometryWithIndependentCollectionDeduplication(bool fieldbook)
        {
            ...
            for (int station = 0; station < payloads.Length; station++)
            {
                ...
                Assert.That(((IKnowledgeMiniGameDefinitionSource)scenes).TryGet(entry.TargetSceneId, out var quiz), Is.True);
                Assert.That(quiz.QuestionCount, Is.GreaterThan(0));
                var content = SessionToken.CreateNew();
