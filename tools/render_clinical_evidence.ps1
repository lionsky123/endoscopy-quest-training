param([string]$Run = 'clinical-evidence', [switch]$LaterCourse, [switch]$Room, [string]$TestFilter = '')
...
try {
    $arguments = @('-batchmode', '-buildTarget', 'Android', '-projectPath', ('"' + $context.MirrorRoot + '"'),
        '-executeMethod', $(if ($Room) { 'BotanicalGardenQR.Bootstrap.Editor.ClinicalRoomVisualCheck.Capture' } elseif ($LaterCourse) { 'BotanicalGardenQR.Bootstrap.Editor.ClinicalCoursePreview.Capture' } else { 'BotanicalGardenQR.Bootstrap.Editor.ClinicalEvidencePreview.Capture' }),
