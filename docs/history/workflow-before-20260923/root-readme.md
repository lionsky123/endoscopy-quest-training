...
## 当前执行入口（2026-09-21，优先于下方历史记录）

实际工程：`D:/quest3/EndoscopyBuild/app`。以0920新剧本为主、旧稿仅补缺；完整路线为大厅→办公室→储存库→候诊区→GI→RESP→洗消室→办公室汇总。医院空间有限，正式入口只使用真实手部原位游玩，主动确认后遮黑切房/切检查点，真实头手保持1:1；观察期间环境固定。

大厅已绑定用户高斯，办公室/诊疗/洗消使用已有模型。旧C09步行、旧六站及开发灰盒不是正式方案；旧源文件保留作追溯，不允许因旧回归测试或本页下方历史描述将其重新接入。当前仍有未完成的储存/候诊场景及部分实物交互，不能称全项目已交付。

按房间先释放再加载，不预加载全库；全程静音，重启新进度。保持Android目标，用户自行编译，不构建、安装或启动APK。原稿医疗记录按参考图加工，不用自绘列表替代。允许生图，但先读[图册](../../../docs/full-script-visuals/ASSET_CATALOG.md)，登记来源、消费者与实际接入状态。

当前实现证据：[原位重构记录](../../../docs/STATIONARY_REBUILD_20260921.md)。完整需求仍以[spec](../../../docs/FULL_SCRIPT_SPEC_20260920.md)、[plan](../../../docs/FULL_SCRIPT_PLAN_20260920.md)和[内容矩阵](../../../docs/FULL_SCRIPT_CONTENT_MATRIX.md)为准，不能缩减为现有目录页面。

## 历史阶段记录（保留追溯，不作为当前接入指令）
...
2026-09-21 晚间更新：默认入口已转为**原地体验**，覆盖本文下方历史步行要求。大厅运行时加载已交付高斯；各房间直接进入最新剧本检查目录，切检查点及换房均手部主动确认、遮黑切换，不要求实际走到光圈/房门。旧C09代码、课程和素材保留。工程改动、资源复用和未完成内容见 [原地重构记录](../../../docs/STATIONARY_REBUILD_20260921.md)。没有构建、安装或启动APK；源码改动不证明设备上旧包已更新。
